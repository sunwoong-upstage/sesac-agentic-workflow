...
from agent.graph import run_travel_planning
...
def target(inputs: dict) -> dict:
    ...
    documents = f"{retrieved_context}\n\n{web_search_info}".strip()
...
def create_dataset():
    ...
    dataset = client.create_dataset(
        ...
        description="여행 계획 에이전트 Q&A 평가 데이터셋",
    )
...
def run_evaluation():
    ...
    experiment_results = evaluate(
        ...
        data=DATASET_NAME,
        evaluators=[correctness, groundedness, concision],
        experiment_prefix="travel-agent-eval",
        metadata={
            "model": "solar-pro2",
            "version": "1.0",
        },
...
if __name__ == "__main__":
    print(f"\n{'#'*60}")
    print(f"# Travel Planning Agent Evaluation")
    print(f"{'#'*60}")
